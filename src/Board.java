...
public class Board {
    ...
    public boolean equals(Object o) {
        ...
        if(o instanceof Board){
            if(this.nx == ((Board) o).nx && this.ny==((Board) o).ny && this.targetRobot == ((Board) o).targetRobot && this.targetPosition == ((Board) o).targetPosition && this.nRobots==((Board) o).nRobots){
                for(int i = 0; i<nRobots; i++){
                    if(this.robots[i] != ((Board) o).robots[i]){
                        return false;
                    }
                }
                return true;
    ...
    public int hashCode() {
        /* TODO */
        int hash = 23 * this.nx + this.ny;
        hash = hash*23 + targetPosition.hashCode();
        hash = hash*23 + nRobots;
        for (int n = 0; n < this.nRobots; n++) {
            hash = hash*23+(n+1)* robots[n].hashCode();
        }
        hash = hash *23 + targetRobot;
        return hash;
    }
